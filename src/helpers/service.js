...
import {getSystemName, clearStorage} from './utils'

export const logout = () => {
    const {PREFIX, hash} = window.location;
    ...
    if(getSystemName() === 'KYZ'){
        location.href = '/protfolio/#/index';
    } else {
        location.href=`${encodeURI(PREFIX)}/login?redirectForm=${encodeURIComponent(hash.slice(1))}`
    }
}
